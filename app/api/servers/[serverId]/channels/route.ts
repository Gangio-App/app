...
const GLOBAL_TIMEOUT_MS = 15000; // 15 seconds max for the entire function
const QUERY_TIMEOUT_MS = 5000; // 5 seconds for individual queries
...
export async function GET(
  ...
  const dbOperationsPromise = async (): Promise<ChannelsResponse> => {
    try {
      ...
      // Get all categories for the server with improved query
      // Try both the original serverId and the server._id.toString()
      const categoriesQuery = {
        $or: [
          { serverId: serverId },
          { serverId: serverIdForQueries },
          // Also try with ObjectId if it's valid
          ...(isValidObjectId(serverId) ? [{ serverId: new ObjectId(serverId) }] : []),
          ...(isValidObjectId(serverIdForQueries) ? [{ serverId: new ObjectId(serverIdForQueries) }] : [])
        ]
      };
      
      console.log(`[Server Channels API] Categories query:`, JSON.stringify(categoriesQuery));
      ...
      // Create the actual query promise with improved options
      const categoriesQueryPromise = getCollection(db, 'categories')
        .find(categoriesQuery)
        .sort({ position: 1 })
        .limit(50) // Increased limit to ensure we get all categories
        .toArray();
      ...
      try {
        ...
        console.log(`[Server Channels API] Found ${categoriesData.length} categories in database`);
      } catch (error) {
        console.error('[Server Channels API] Error fetching categories:', error);
        // Provide default category if query times out
        categoriesData = [{
          id: 'default-category',
          name: 'General',
          serverId: serverId,
        ...
        console.log('[Server Channels API] Using fallback category');
      }
      ...
      // Get all channels for the server with improved query
      // Try both the original serverId and the server._id.toString()
      const channelsQuery = {
        $or: [
          { serverId: serverId },
      ...
      console.log(`[Server Channels API] Channels query:`, JSON.stringify(channelsQuery));
      ...
      const channelsQueryPromise = getCollection(db, 'channels')
        .find(channelsQuery)
        .sort({ position: 1 })
        .limit(100) // Increased limit to ensure we get all channels
        .toArray();
      ...
      try {
        ...
        console.log(`[Server Channels API] Found ${channelsData.length} channels in database:`, 
          channelsData.map(ch => ({ id: ch.id, name: ch.name, type: ch.type })));
      } catch (error) {
        ...
        // Store these IDs in the database for future reference
        try {
          await getCollection(db, 'channels').insertMany([
            {
              id: generalId,
              name: 'general',
              type: 'text',
              serverId: serverId,
              categoryId: categoriesData[0]?.id || 'default-category',
              position: 0,
              createdAt: new Date(),
              updatedAt: new Date(),
            },
          ...
          console.log('[Server Channels API] Created fallback channels in database');
        } catch (insertError) {
          console.error('[Server Channels API] Failed to insert fallback channels:', insertError);
        }
        ...
        console.log('[Server Channels API] Using fallback channels');
      }
