  ...
  return (
    ...
    >
      <div className="bg-gray-800/80 backdrop-blur-sm rounded-lg border border-gray-700 overflow-hidden shadow-lg">
        <div 
          className="bg-gray-700/60 p-2 sm:p-3 flex flex-col sm:flex-row items-stretch sm:items-center sm:justify-between cursor-grab" 
          {...attributes} 
          {...listeners}
        >
          <span className="font-medium text-gray-200 mb-2 sm:mb-0">{category.name}</span>
...
function SortableChannel({ channel, onEdit, onDelete }: {
  ...
  return (
    <div 
      ref={setNodeRef} 
      style={style} 
      className="bg-gray-800/70 hover:bg-gray-700/70 p-1.5 sm:p-2 rounded-md flex flex-col sm:flex-row items-start sm:items-center sm:justify-between cursor-grab"
      {...attributes} 
      {...listeners}
    >
      <div className="flex items-center mb-1.5 sm:mb-0">
        {getChannelIcon()}
        <span className="text-gray-300 text-sm ml-2">{channel.name}</span>
      </div>
                        ...
                        >
                          ...
                          <div className="space-y-1 p-1">
                            ...
                            >
                              <SortableContext 
                                items={channels
                                  .filter(ch => ch && ch.categoryId === category.id)
                                  .map(ch => ch.id)}
                                strategy={verticalListSortingStrategy}
                              >
                                {channels
                                  .filter(channel => channel && channel.categoryId === category.id)
                                  .map(channel => (
                                    <SortableChannel 
                                      key={channel.id}
                                      channel={channel}
                            ...
                            {channels.filter(channel => channel && channel.categoryId === category.id).length === 0 && (
